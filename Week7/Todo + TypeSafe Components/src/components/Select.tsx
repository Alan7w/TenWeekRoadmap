...
export const SimpleSelect = forwardRef<HTMLDivElement, SelectProps<unknown>>(
  function SimpleSelect<T>(props: SelectProps<T>, ref: React.Ref<HTMLDivElement>) {
    const {
      placeholder = 'Select an option...',
      size = 'md',
      variant = 'default',
...
function SelectTrigger({ 
  ...
  const triggerClasses = useMemo(() => {
    const classes = [
      ...
      'text-left', 'bg-white', 'border', 'rounded-md', 'cursor-pointer',
  ...
  return (
    ...
    >
      {children}
      <ChevronDownIcon className={`ml-2 h-4 w-4 transition-transform ${state.isOpen ? 'rotate-180' : ''}`} />
    </button>
  );
}
...
function SelectItem<T>({ 
  ...
  const isHighlighted = state.filteredOptions.findIndex(opt => opt.value === value) === state.highlightedIndex;
