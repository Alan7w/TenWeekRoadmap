...
const Shoes: React.FC = () => {
  const { 
    ...
    updateSort,
    isLoading 
  } = useProductFilters('shoes');

  // Function to clear custom price inputs in FilterSidebar
  const [clearCustomPriceTrigger, setClearCustomPriceTrigger] = useState(0);
  
  const handleClearAllFilters = () => {
    clearFilters();
    setClearCustomPriceTrigger((prev: number) => prev + 1); // Trigger custom price clear
  };
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex gap-8">
          ...
          <aside className="w-64 shrink-0">
            <div className="sticky top-6">
              <FilterSidebar
                ...
                key={clearCustomPriceTrigger} // Force re-render to clear inputs
              />
          ...
          <main className="flex-1">
            <ProductGrid 
              key={`shoes-${filteredProducts.length}`}
